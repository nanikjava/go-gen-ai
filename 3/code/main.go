...
func homepage(host *string) func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		log.Println("wss://" + *host + "/live")
		// Execute the template, passing the WebSocket URL to it.
		err = tmpl.Execute(w, "wss://"+*host+"/live")
		if err != nil {
...
func main() {
	host := flag.String("host", "localhost", "Hostname for connecting from websocket to server")
	flag.Parse()
	...
	// Start HTTP server.
	log.Printf("listening on port %s for host %s", port, *host)
